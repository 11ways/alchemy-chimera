...
alchemy.create('ChimeraModule', function ModelEditorChimeraModule() {
	...
	this.actions = {
		index: {
			route: '/index',
			title: __('chimera', 'Back to :modelTitle overview'),
			class: 'btn-primary',
			icon: 'list',
			flags: ['read']
		},
		...
		add: {
			route: '/add',
			title: __('chimera', 'Add a new :newItem'),
			icon: 'plus',
			flags: ['create']
		},
		remove: {
		...
		export: {
			route: '/export',
			title: __('chimera', 'Export these results'),
			class: 'btn-primary',
			icon: false,
			flags: ['read'],
			history: false
	 ...
	 * @version  0.0.1
	 */
	this.modelAssocOptions = function modelAssocOptions(render) {
		var ModelViews = this.getView('ModelEditor');
		ModelViews.modelAssocOptions(render, this);
	};
	 ...
	 * @version  0.0.1
	 */
	this.custom = function custom(render) {
		var ModelViews = this.getView('ModelEditor');
		ModelViews.custom(render, this);
	};
		...
		for (modelName in models) {
			...
			if (alchemy.models[modelName] && alchemy.models[modelName].admin !== false) {
				...
				if (data.settings && data.settings.title) {
					temp = alchemy.pickTranslation(this.render, data.settings.title, false);
