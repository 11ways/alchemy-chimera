...
alchemy.classes.BaseClass.extend(function ChimeraModule() {
	...
	this.getBaseUrl = function getBaseUrl(parameters) {
		...
		var result = '/' + alchemy.plugins.chimera.routename + '/' + this.routeName;
	...
	this.beforeAction = function beforeAction(next, render) {
		...
		if (!render.viewVars.chimeraModuleActions) {
			render.viewVars.chimeraModuleActions = this.actions;
		}

		if (!render.viewVars.chimeraModuleActionLists) {
	...
	this.getPieceTitle = function getPieceTitle(data) {
		...
		if (data.settings.module) {
			title = ' "' + String(data.settings.module).titleize() + '"';
		} else {
			title = ': New Entry';
		}

		result = 'Chimera Module' + title;
		return result;
	};
	...
	this.configureMenu = function configureMenu(mit, data, settings, callback) {
		...
		if (data.settings && data.settings.parent) {
			parent = data.settings.parent;
		}

		if (grouped) {
			groupid = 'Chimera' + this.name + 'GroupedModel';
...
ChimeraModules.default_chimera_module = new alchemy.classes.ChimeraModule();