...
BelongsTo.setMethod(function actionValue(actionType, record, callback) {
	...
	model.find('all', find_options, function gotData(err, results) {
		...
		let options = [model.displayField, 'title', 'name', '_id'],
		    option;
		...
		if (response) {
			let field = item.$model.getField(used_field);

			if (field && field.is_translatable) {
				let conduit = record.conduit;
				response = alchemy.pickTranslation(conduit, response).result;
			}
