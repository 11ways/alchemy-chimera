...
BelongsTo.setMethod(function actionValue(actionType, record, callback) {
	...
	model.find('list', find_options, function gotData(err, results) {

		var response,
		    item;

		console.log('Found belongs to action value in', Date.now() - start, 'ms', start, Date.now());
