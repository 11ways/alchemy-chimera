...
alchemy.create('ModelEditorField', function Select2MEF() {
	...
	this.options = function options(value, callback) {
		...
		Array.cast(assocModel.displayField).forEach(function(name) {
			sort[name] = 1;
		});
		...
		assocModel.find('all', options, function (err, items) {
			...
			for (i = 0; i < items.length; i++) {
				...
				if (assocModel.formatDisplay) {
					display = assocModel.formatDisplay(item, items[i], fallbacks);
				} else {
					display = assocModel.getDisplayTitle(items[i], fallbacks);
				}
	...
	this.save = function save(callback) {
		...
		if (this.value === '') {
			...
			for (i = 0; i < this.value.length; i++) {
				...
				if (typeof this.value[i] === 'string' && this.value[i].isObjectId()) {
	...
	this.filter = function filter(condition, value, callback) {
		...
		value.forEach(function eachValue(val) {

			var tempValue;

			// If result has already been filled in, stop the forEach
			if (result) {
				return;
			}
			...
			switch (condition) {
				...
				case 'notlike':
				case 'like':
					tempValue = that.anyWord(val);
					break;

				default:
					tempValue = new RegExp(String(val).diacriticPattern());
			}
		...
		// If result is already set, return it now
		if (result) {
			return callback(result);
		}

		// Look up the ids in the associated model
		assocModel.find('all', {conditions: conditions}, function gotAssocResults(err, results) {
