...
alchemy.create('ModelEditorField', function Select2MEF() {
	...
	this.options = function options(value, callback) {
		...
		if (body.q) {
			...
			if (conditionFields.length > 1) {
				...
				options.conditions[conditionFields[0]] = tempCondition;
			}
		}
	...
	this.filter = function filter(condition, value, callback) {
		...
		switch (condition) {
			...
			case 'notlike':
			case 'like':
				tempCondition = new RegExp('.*?' + String(value).diacriticPattern(true) + '.*?', 'i');
				break;

			default:
				tempCondition = new RegExp(String(value).diacriticPattern());
		}
		...
		if (conditionFields.length > 1) {
			...
			conditionFields.filter(function(fieldname) {
				orconditions[fieldname] = tempCondition;
			});

			conditions['$or'] = orconditions;
		} else {
			conditions[conditionFields[0]] = tempCondition;
		}

		// Set the conditions for the associated find
		conditions[assocModel.displayField] = tempCondition;
