...
alchemy.create('ChimeraView', function JsonAddChimeraView() {
	...
	this.build = function build(render, module, options) {
		...
		render.viewVars.showActions = 'paginate';
		...
		if (render.get) {
			...
			for (key in model.blueprint) {
				field = model.blueprint[key];
				...
				newObject[original][key] = value;
			}

			render.viewVars.fields = fields;
			render.viewVars.__current__ = newObject;
			render.viewVars.item = newObject;
