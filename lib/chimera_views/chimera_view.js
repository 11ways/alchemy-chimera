var ChimeraViews = alchemy.shared('Chimera.views');
 ...
 * @version  0.0.1
 */
alchemy.classes.BaseClass.extend(function ChimeraView() {
	...
	this.__extended__ = function __extended__(parent, child) {
		...
		// Extract the name
		var name     = child.name.replace(/ChimeraView$/, ''),
		    typeName = name.underscore();

		child.prototype.viewName = name;
		...
		if (!child.prototype.extendonly) {
			ChimeraViews[typeName] = new child();
		}
	};
