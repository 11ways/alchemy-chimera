...
alchemy.create('ChimeraView', function ModelEditorChimeraView() {
	...
	this.index = function index(render, module, options) {
		...
		this.getModelIndexFields(model, function(fieldInfo) {
			...
			if (filtersRaw && andorRaw && (filters = JSON.parse(filtersRaw))){
				...
				for(var i=0; i<filters.length; i++){
					if(filters[i].name === 'global_search'){
						globalfilter = true;
						andor = render.viewVars.andor = 'or';
						var globalcondition = filters[i].condition;
						var globalvalue = filters[i].value;
						delete(filters[i]);
					}
				}
				
				if(globalfilter){
					for(var filtername in baseFieldObject){
						var filter = {};
						filter.name = filtername;
						filter.condition = globalcondition;
						filter.value = globalvalue;
						filters.push(filter);
					}
				}

				filters.filter(function(filter, index) {

					var fieldConfig = baseFieldObject[filter.name],
					    fieldType;
					...
					fieldType = module.getField(fieldConfig.type);
					...
					filterTasks[filterTasks.length] = function(next_task) {
						...
						fieldType.getFilterValue({
							...
							if(!target[filter.name]){
								target[filter.name] = [];
							}

							// Add the value to the array (merge arrays if an array is given)
							target[filter.name] = target[filter.name].concat(match_value);
