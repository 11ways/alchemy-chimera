...
alchemy.create('ChimeraView', function ModelEditorChimeraView() {
	...
	this.getFilterConditions = function getFilterConditions(render, model, module, callback) {
		...
		if (Object.exists(render, 'req.body.data')) {
			data = Object.path(render, 'req.body.data');
		} else if (Object.exists(render, 'req.query.filter')) {
			try {
				// If we get data via a query string we need to try to parse the json
				data = JSON.parse(Object.path(render, 'req.query.filter'));
			} catch(e) {}
		}
		...
		render.viewVars.searchData = data;
		...
		fieldConditions.forEach(function(field, index) {
			...
			config = that.getFieldConfig(info, model);
	...
	this.export = function exprt(render, module) {
		...
		// This could take a while, don't let the request timeout
		render.res.connection.setTimeout(0);

		log.verbose('EXPRT: Getting filter conditions');

		if (typeof render.req.url == 'string') {
			checksum = render.req.url.checksum();
		} else {
			log.verbose('Url was not a valid string');
			checksum = Date.now();
		}

		console.time('Excel Export (Request ' + checksum + ')');

		this.getFilterConditions(render, model, module, function gotConditions(err, result) {
			...
			if (!result.conditions || Object.isEmpty(result.conditions)) {
				...
				log.verbose('EXPRT: No filters were set, redirecting user to ' + JSON.stringify(url));
				render.redirect(url);
				return;
			}
			...
			log.verbose('EXPRT: Querying database');
			//findOptions.debug = true;
			...
			model.find('all', findOptions, function(err, items) {
				...
				log.verbose('EXPRT: Got ' + items.length + ' records');

				// Iterate over every record
				items.forEach(function eachRecord(item, index) {

					// Process all the fields in this record
					tasks[index] = function processFields(next) {
						...
						// Queue every field to export
						Object.each(todo, function eachTodoField(value, fieldKey) {

							var options,
							    field;

							if (valueCache[value] !== undefined) {
								results[fieldKey] = valueCache[value];
								return;
							}
							...
							field = model.getModelEditorField(fieldKey);
							...
							itemTasks[fieldKey] = function(next_task) {
								field.prepareExport(options, function(err, result) {

									// Store this value in the request cache
						...
						async.parallelLimit(itemTasks, 9, function finishedRecord(err, new_results) {

							if (err) {
								return next(err);
							}

							Object.assign(results, new_results);
				...
				log.verbose('EXPRT: Executing parallel tasks: ' + tasks.length);
				...
				async.parallelLimit(tasks, 5, function(err, result) {
					...
					log.verbose('EXPRT: Going to create Excel file');
					...
					columns.forEach(function(key, index) {
						...
						if (excelFields.hasOwnProperty(key)) {
							name = excelFields[key];
						} else {
							field = model.getModelEditorField(key);
							name = field.info.field;
						}
					...
					log.verbose('EXPRT: Saving workbook');

					workbook.save(function(err) {

						console.timeEnd('Excel Export (Request ' + checksum + ')');

						if (!err) {
							log.verbose('EXPRT: Serving saved workbook to user');
							render.serveFile(filePath, 'export-' + model.modelName + '.xlsx', true);
						} else {
							log.verbose('EXPRT: Got error "' + err + '"');
