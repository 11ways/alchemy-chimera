...
alchemy.create('ChimeraView', function ModelEditorChimeraView() {
	...
	this.assocOptions = function assocOptions(render, module) {
		...
		fields = this.getGroupFields(groups, fieldName);
	...
	this.index = function index(render, module, options) {
		...
		fieldInfo = this.getModelIndexFields(model);
	...
	this.prepareSave = function prepareSave(data, options, callback) {
		...
		if (!options.fields && options.dataInfo) {
			...
			if (!options.groups) {
				// Get a (cloned) groups object
				options.groups = this.getModelGroups(options.dataInfo);
			}

			// Extract all the fields by reference
			options.fields = this.getGroupFields(options.groups);
		}
	 ...
	 * @param    {Model}   model
	 */
	this.preloadEditorSettings = function preloadEditorSettings(model) {
		...
		if (!model._modelEditorGroups) {
			...
			if (!model.modelEdit) {
				...
				this.finalizeGroups(translationGroups, model, model._modelEditorGroups);
			} else {

				// Convert the user given configuration to something useable
				model._modelEditorGroups = this.convertSettings(model.modelEdit, model);
			}
		}

		indexList = model._modelFieldInfo.index.fields;
		baseList = model._modelFieldInfo.edit.base;
		...
		return {
			groups: model._modelEditorGroups,
			fields: model._modelFieldInfo
		};
	};
	 ...
	 * @param    {Model}   model
	 */
	this.getModelIndexFields = function getModelIndexFields(model, piece) {

		var data;

		if (typeof model.preloadEditorSettings == 'function') {
			data = model.preloadEditorSettings(this, piece);
		} else {
			data = this.preloadEditorSettings(model, piece);
		}

		return alchemy.cloneSafe(data.fields.index.fields);
	};
	 ...
	 * @param    {Model}   model
	 */
	this.getModelGroups = function getModelGroups(model, piece) {
		...
		return alchemy.cloneSafe(data.groups);
	};
		...
		for (groupName in result) {
			for (i = 0; i < result[groupName].fields.length; i++) {
				...
				if (!duplicates[entry.field]) {
					model._modelFieldInfo.edit.base.push(entry);
