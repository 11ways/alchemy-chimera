		...
		this.getModelIndexFields(model, function(fieldInfo) {
			...
			if(filtersRaw && andorRaw){
				...
				for(var i in filters){
					...
					var fieldInst = model.blueprint[field];
					var foreignInst = model.foreignKeys[field];
								...
								tasks[tasks.length] = function foreignFinder(next) {
									...
									if (condition === 'like') {
										if(andor == 'or'){
											...
											foreignAnd[assocModel.displayField] = like;
										}
									} else if (condition === 'is') {
										if(andor == 'or'){
											foreignOr[assocModel.displayField] = value;
										} else if(andor == 'and') {
											foreignAnd[assocModel.displayField] = value;
										}
