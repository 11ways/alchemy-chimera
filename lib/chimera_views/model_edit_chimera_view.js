...
alchemy.create('ChimeraView', function ModelEditorChimeraView() {
				...
				(function(model, conditions) {
					tasks.getOriginalData = function getOriginalData(done) {
						...
						model.find('first', {conditions: conditions}, function(err, item) {
							...
							if (item) {
								options.originalData = item[model.modelName];
							} else {
	 ...
	 * @param    {Model}   model
	 */
	this.getModelIndexFields = function getModelIndexFields(model) {
		this.preloadEditorSettings(model);
		return alchemy.cloneSafe(model._modelFieldInfo.index.fields);
	};
